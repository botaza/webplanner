...
// FIXED: Touch screen edit button now properly stops propagation and registers click
...
container.innerHTML =  `<div class=` `"text-center text-zinc-500 py-10` `"` `> ` `<div class=` `"text-4xl mb-2` `"` `>🍃` `</div` `> ` `<div` `>No expenses yet.` `</div` `> ` `</div` `>` ;
return;
}
            ...
            return `
               <div class= "bg-zinc-900/30 rounded-xl p-3 flex justify-between items-center text-sm " >
                 ...
                 <div class= "flex items-center gap-2 " >
                   ...
                   <button type="button" ontouchstart="event.stopPropagation()" onclick="window.editExpense('${exp.id}'); event.stopPropagation()"
                          class="expense-action-btn text-zinc-400 hover:text-white text-base transition px-1"
                          title="Edit">✏️</button>
                   <button type="button" ontouchstart="event.stopPropagation()" onclick="window.deleteExpense('${exp.id}'); event.stopPropagation()"
                          class="expense-action-btn text-red-400 hover:text-red-300 text-base transition px-1"
                          title="Delete">🗑</button>
                 </div >
               </div >
            `;
...
if (!list || list.length === 0) {
container.innerHTML =  `<div class=` `"text-center text-zinc-500 py-4` `"` `> ` `<div class=` `"text-2xl mb-1` `"` `>🔍` `</div` `> ` `<div class=` `"text-sm` `"` `>No data found for this view.` `</div` `> ` `</div` `>` ;
return;
}
const sorted = [...list].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
const groupedByMonth = {};
...
section.innerHTML = `<div class="text-xs text-zinc-500 uppercase font-medium tracking-wide mb-3"> Entries by Category </div> ${blocksHTML}`;
