...
async function saveEvent() {
    ...
    const base = {
        ...
        recurrence,
        completed: false // new events are incomplete by default
    };
...
async function markComplete(id) {
    const ev = state.eventsData.find(e => e.id == id);
    if (!ev) return;
    const newStatus = !ev.completed;
    await api('complete_event', {id, completed: newStatus});
    ev.completed = newStatus; // update local state
    loadPlanner();
    loadDashboard();
}
...
export { loadPlanner, saveEvent, editEvent, updateEvent, deleteEvent, markComplete, showAddEventModal, onRecurrenceChange, updateOccurrencePreview };