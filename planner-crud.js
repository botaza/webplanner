...
import { applyPlannerFilter, renderPlannerHashtagFilter } from './planner-filter.js';
...
async function markComplete(id) {
    const ev = state.eventsData.find(e => e.id == id);
    if (!ev) return;
    const completed = !ev.completed;  // toggle completion
    await api('complete_event', {id, completed});
...
async function loadPlanner() {
    ...
    state.eventsData = data.map(e => e); // keep all events including completed
...
export { loadPlanner, saveEvent, editEvent, updateEvent, deleteEvent, markComplete };